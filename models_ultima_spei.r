...
for (i in 1:sim) {
    ...
    #creating model
    svmfit=svm(labs ~ white + black, data=train, kernel=kern,
               cost=cst, coef0= co, gamma=gam,
               #cost=2, coef0= 1, gamma=0.001, #degree=2,
...
ultima<-(rbind(train_results, valid_results))
...
models<-( rep(rownames(ultima)[1:3], 8 ) )
set<-( rep(c(rep("Training", 3), rep("Validation", 3)), 4) )
...
ggsave(filename="plots/OverallAcc_nlm.png", plot=ultima_plot,
       width=9, height=7)
...
mydata2<-rbind(tris, squs, pens, pens2, hexs, hexs2)
